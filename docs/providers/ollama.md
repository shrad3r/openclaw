...
<AccordionGroup>
  ...
  <Accordion title="Lean local model profile">
    ...
    `localModelLean` removes heavyweight optional tools from the direct agent surface, including browser, cron, message, media-generation, voice, and PDF tools, but it does not change Ollama's runtime context or thinking mode. Pair it with explicit `params.num_ctx` and `params.thinking: false` for small Qwen-style thinking models that loop or spend their response budget on hidden reasoning.
